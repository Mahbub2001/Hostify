...
-- -----------------------------Create HostAdvertisements table---------------------------------
        ...
        CREATE TABLE HostAdvertisements (
            ...
            DiscountPercentage DECIMAL(5, 2) CHECK (DiscountPercentage >= 0 AND DiscountPercentage <= 100) DEFAULT 0,
...
SET @bookingID = 12;  -- id of BookingID

-- Retrieve booking and advertisement details including discount
SELECT 
    b.TotalPrice, 
    ha.DiscountPercentage, 
    ha.DiscountActive,
    CASE 
        WHEN ha.DiscountActive = 1 THEN b.TotalPrice * ha.DiscountPercentage / 100
        ELSE 0
    END AS DiscountAmount,
    CASE 
        WHEN ha.DiscountActive = 1 THEN b.TotalPrice - (b.TotalPrice * ha.DiscountPercentage / 100)
        ELSE b.TotalPrice
...
FROM 
    Bookings b
JOIN 
    HostAdvertisements ha ON b.AdvertID = ha.AdvertID
WHERE 
...
-- Insert the transaction with the discount details
INSERT INTO Transactions (BookingID, PaymentAmount, PaymentStatus, DiscountPercentage, DiscountAmount)
VALUES (@bookingID, @paymentAmount, 'Confirmed', @discountPercentage, @discountAmount);

-- Update advertisement availability to 'No'
UPDATE HostAdvertisements ha
JOIN Bookings b ON ha.AdvertID = b.AdvertID
SET ha.Availability = 'No'
...
-- -----------------Add Discount--------------------------
UPDATE HostAdvertisements
SET DiscountPercentage = 20.00, -- Example discount percentage
    DiscountActive = TRUE
WHERE AdvertID = 91; -- ID of the advertisement to update
